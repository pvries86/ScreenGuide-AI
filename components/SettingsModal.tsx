...
interface SettingsModalProps {
  ...
  onApiKeyDelete: () => Promise<boolean>;
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleDelete = async () => {
    const deleted = await onApiKeyDelete();
    setLocalApiKey('');
    setSaveStatus(deleted ? 'saved' : 'error');
    setTimeout(() => setSaveStatus('idle'), 2000);
  };
      ...
      >
        ...
        <div className="mt-8 flex justify-end gap-2">
            <button
                ...
                disabled={!apiKey && !localApiKey}
                className="px-5 py-2.5 text-sm font-semibold text-red-700 dark:text-red-300 bg-white dark:bg-slate-700 border border-red-300 dark:border-red-600 rounded-lg shadow-sm hover:bg-red-50 dark:hover:bg-red-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
                Remove Key
            </button>
            <button
